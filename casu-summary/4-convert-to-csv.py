...
with open('tmp/concatenated-sum8-files.txt.corrected', 'w') as output, open("tmp/concatenated-sum8-files.txt", 'r') as original:
...
data = ascii.read("tmp/concatenated-sum8-files.txt.corrected", data_start=1, \
...
ascii.write(data, output="tmp/egaps-casu-dqc-by-run.csv", delimiter=",")
